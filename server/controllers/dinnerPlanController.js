...
const createDinnerPlan = async (req, res) => {
  try {
    ...
    } else if (totalGroupSize > 2 && !["direct", "rating"].includes(matchType)) {
      return res.status(400).json({ error: "For 3-5 users, matchType must be 'direct' or 'rating'" });
    }

    // For now, create a placeholder structure for votes
    const votes = []; // This will be populated later after restaurant fetching
    const done = []
    const newPlan = new DinnerPlan({
      creator,
      group,
      budget,
      diningStyle,
      ...
      votes // Add the blank vote structure here
    });
    ...
    res.status(500).json({ error: "Failed to create dinner plan." });
  }
};
